...
class TestStringMethods(unittest.TestCase):
    ...
    def helper_nary_grid(self, board):
        ...
        N = board[0][0]
        expected_nary_const_count = N * 2  # N for rows and N for columns
        cons = csp.get_all_cons()
        nary_count = 0
        for c in cons:
            if len(c.get_scope()) == N:  # All variables in a row or column
                nary_count += 1
        self.assertEqual( expected_nary_const_count, nary_count,
                         "Wrong number of n-ary all different constraints for nary_ne_grid")
    ...
    @unittest.skipUnless(TEST_ENCODINGS, "Not Testing Encodings.")
    def test_ne_grid_1(self):
        board = BOARDS[0]
        self.helper_nary_grid(board)

    @unittest.skipUnless(TEST_ENCODINGS, "Not Testing Encodings.")
    def test_ne_grid_2(self):
        board = BOARDS[1]
        self.helper_nary_grid(board)

    @unittest.skipUnless(TEST_ENCODINGS, "Not Testing Encodings.")
    def test_ne_grid_3(self):
        board = BOARDS[2]
        self.helper_nary_grid(board)

    @unittest.skipUnless(TEST_ENCODINGS, "Not Testing Encodings.")
    def test_ne_grid_4(self):
        board = BOARDS[3]
        self.helper_nary_grid(board)

    @unittest.skipUnless(TEST_ENCODINGS, "Not Testing Encodings.")
    def test_ne_grid_5(self):
        board = BOARDS[4]
        self.helper_nary_grid(board)

    @unittest.skipUnless(TEST_ENCODINGS, "Not Testing Encodings.")
    def test_ne_grid_6(self):
        board = BOARDS[5]
        self.helper_nary_grid(board)
