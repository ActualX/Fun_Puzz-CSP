...
boards1 = [ [[3],[11, 2],[11,21,3,0],[12,22,2,1],[13,23,33,6,3],[31,32,5,0]],
[[4],[11,21,6,3],[12,13,3,0],[14,24,3,1],[22,23,7,0],[31,32,2,2],[33,43,3,1],[34,44,6,3],[41,42,7,0]],
[[5],[11,21,4,1],[12,13,2,2],[14,24,1,1],[15,25,1,1],[22,23,9,0],[31,32,3,1],[33,34,44,6,3],[35,45,9,0],[41,51,7,0],[42,43,3,1],[52,53,6,3],[54,55,4,1]],
[[6],[11,21,11,0],[12,13,2,2],[14,24,20,3],[15,16,26,36,6,3],[22,23,3,1],[25,35,3,2],[31,32,41,42,240,3],[33,34,6,3],[43,53,6,3],[44,54,55,7,0],[45,46,30,3],[51,52,6,3],[56,66,9,0],[61,62,63,8,0],[64,65,2,2]],
...
class TestStringMethods(unittest.TestCase):
    ...
    def helper_prop_binary(self, board, prop=prop_FC):
        csp, var_array = binary_ne_grid(board)
        solver = BT(csp)
        solver.bt_search(prop)
        self.assertTrue(check_diff(var_array, board), "Repeated value in a row or column!")

    def helper_prop_nary(self, board, prop=prop_FC):
    ...
    def helper_nary_grid(self, board):
        new_b = []
        for sub_list in board:
            new_b.append(list(sub_list))
        csp, _ = nary_ad_grid(new_b)
        diff_const_count = 2 * board[0][0]# number of all binary diff constraints
        cons = csp.get_all_cons()
        nary_count = 0 # number of binary constraints
        for c in cons:
            if len(c.get_scope()) == board[0][0]:
                nary_count += 1
        self.assertEqual(nary_count, diff_const_count, "Wrong number of nary not equal constraints for binary_ne_grid!")
...
#=====test bne
    ...
    @unittest.skipUnless(TEST_ENCODINGS, "Not Testing Encodings.")
    def test_nary_grid_5(self):
        board = BOARDS[4]
        self.helper_nary_grid(board)

    @unittest.skipUnless(TEST_ENCODINGS, "Not Testing Encodings.")
    def test_nary_grid_6(self):
        board = BOARDS[5]
        self.helper_nary_grid(board)
#=====test nary
...
#====================== test bi and na
    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Propagators and Encodings.")
    def test_helper_prop_binary1(self):
        board = BOARDS[0]
        self.helper_prop_binary(board)

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Propagators and Encodings.")
    def test_helper_prop_binary2(self):
        board = BOARDS[1]
        self.helper_prop_binary(board)

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Propagators and Encodings.")
    def test_helper_prop_binary3(self):
        board = BOARDS[2]
        self.helper_prop_binary(board)

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Propagators and Encodings.")
    def test_helper_prop_binary4(self):
        board = BOARDS[3]
        self.helper_prop_binary(board)

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Propagators and Encodings.")
    def test_helper_prop_binary5(self):
        board = BOARDS[4]
        self.helper_prop_binary(board)

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Propagators and Encodings.")
    def test_helper_prop_binary6(self):
        board = BOARDS[5]
        self.helper_prop_binary(board)

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Propagators and Encodings.")
    def test_helper_prop_nary1(self):
        board = BOARDS[0]
        self.helper_prop_nary(board)

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Propagators and Encodings.")
    def test_helper_prop_nary2(self):
        board = BOARDS[1]
        self.helper_prop_nary(board)
    #这个test过不去，不知道为什么.
    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Propagators and Encodings.")
    def test_helper_prop_nary3(self):
        board = BOARDS[2]
        self.helper_prop_nary(board, prop_FI)

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Propagators and Encodings.")
    def test_helper_prop_nary4(self):
        board = BOARDS[3]
        self.helper_prop_nary(board, prop_FI)

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Propagators and Encodings.")
    def test_helper_prop_nary5(self):
        board = BOARDS[4]
        self.helper_prop_nary(board, prop_FI)

    @unittest.skipUnless(TEST_PROPAGATORS and TEST_ENCODINGS, "Not Testing Propagators and Encodings.")
    def test_helper_prop_nary6(self):
        board = BOARDS[5]
        self.helper_prop_nary(board, prop_FI)
#===================
